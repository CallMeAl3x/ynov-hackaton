  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert("Failed to delete character: " + (result.error || "Unknown error"));
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto py-12 px-4">
      ...
      <div className="space-y-8 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-700 leading-relaxed text-base">{character.description}</p>
        </div>

        {/* Relationships Section */}
        {character.relationships && (
          <div className="pt-6 border-t border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900 mb-3">Relations</h2>
            <p className="text-gray-700 leading-relaxed text-base">{character.relationships}</p>
          </div>
        )}
      ...
      <EditCharacterModal
        ...
        onSuccess={(updatedData) => {
          setCharacter({
            ...
            role: updatedData.role as "PROTAGONIST" | "ANTAGONIST" | "SECONDARY" | "MINOR",
          });
        }}
      />
    </div>
